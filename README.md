...
- **OAuth connection to Google Drive**, authorised once from `/admin`. The
  refresh token is encrypted at rest and renewed automatically afterwards.
...
- **EXIF**: capture date, camera, lens, aperture, shutter speed, ISO,
  geolocation. Chronological ordering on the real capture date.
- **Per-photo comments**, in a side panel, with one level of reply. Everyone
  signs with their own name: since a credential may be shared by a whole
  household, the writer declares a name and an address at write time, and a code
  received by email confirms the address. An administrator can hide a comment
  from `/admin`, and make it visible again.
- **Email notifications**: the moderation address set in `/admin` is told about
  new comments, and the author of a thread about replies to it. Every message
...
For development, or to see the application running without a VPS, a domain name
or a Google account. Node ≥ 22 and pnpm are all that is needed.
...
Finally, **copy the public key into `deploy/cloud-init.yaml`**, replacing the
...
3. **Publish the application.** This step is required: while it stays in "Testing"
   status, Google **expires the refresh token after 7 days** and reconnection is
   needed every week.
...
| Enable comments                | `SMTP_URL` and `MAIL_FROM` in `.env` (see "Email" at install time) — without a sending server nobody can identify themselves                                                  |
...
| Back up                        | `./deploy/backup.sh` — the `gdv-data` volume **and** the `.env`, see "Backup" below. `gdv-cache` is regenerable                                                               |
...
**The `gdv-data` volume now holds the accounts**: it is that volume, and only
that one, which must be backed up.
...
- **Security headers on every response**: `Content-Security-Policy`
  (`script-src 'self'` — a `<script>` slipped into an album title or a comment
  does not execute), `X-Content-Type-Options`, `Referrer-Policy`,
  `frame-ancestors 'none'`, and `Strict-Transport-Security` as soon as
...
new Google consent. `deploy/backup.sh` takes both.
