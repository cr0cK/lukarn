...
| `APP_NAME`                    | `Photos`                                      | Instance name, as a **bootstrap value only** (D260813c). It seeds the `instanceName` setting while the database has none saved — a fresh installation, or the first start after this change on an instance that already had one — and is **ignored from then on**: the name is edited from `/admin/identity`, where the colour and the logo are. It still lives here because the name must exist before an account does, the first page served being the sign-in screen. Empty ⇒ refuses to start, so as not to display a nameless application.                                                                                                                                                                                                                                                                                                                   |
| `APP_VERSION`                 | `dev`                                         | Version this instance runs, shown under "Powered by Lukarn" and compared against the release feed. **Set by the `Dockerfile`, not by compose**: the release workflow passes the tag as a build argument, so the image carries the version it was built from and an instance cannot claim another one. Everything else — a local build, `pnpm dev`, the compose overlay that builds from source — stays at `dev`, which is never compared to anything: no build outside a release is ever told it is out of date (D260815).                                                                                                                                                                                                                                                                                                                                        |
| `UPDATE_CHECK_URL`            | GitHub releases feed for this repository      | Where "does a newer version exist?" is asked. **An empty string disables it**: nothing leaves the machine, and the interface shows the running version without offering anything. Asked at most once every six hours, only while an **administrator** is looking at the answer, and never from a build whose version cannot be read. An invalid URL stops startup rather than leaving an instance that silently never reports an update — indistinguishable from one that is up to date. See [05](./05-api.md) and D260815.                                                                                                                                                                                                                                                                                                                                       |
| `DEFAULT_LOCALE`              | `en`                                          | Language of what the server writes **without knowing its reader**: the email sent to the moderation address, which belongs to no identity, and a page requested by a browser asking for neither supported language. It decides nothing for a visitor — their browser announces its own language and the account menu overrides it (see [07](./07-frontend.md)). Accepted values: `en`, `fr`; anything else falls back to `en` without failing startup, because a typo must not stop an instance over the choice between two languages.                                                                                                                                                                                                                                                                                                                            |
...
| `STORAGE_LOCAL_ROOT`          | empty                                         | The one directory a **local folder** storage may read, as a path the server sees — `/photos` under Docker, where the commented `./photos:/photos:ro` volume mounts it. **An empty string means the kind is unavailable**: `/admin` does not offer it, because there is no folder this instance was given. It is a fence rather than a default: a connection chooses a subpath under it and never an absolute path, so nobody holding an administrator password can point an album at `/etc`, and a symlink leading out of the tree is refused rather than followed. See [04](./04-security-and-access.md) and D260816d.                                                                                                                                                                                                                                           |
...
| `DATA_DIR`                    | `./data`                                      | Contains `lukarn.db` and `branding/` — the uploaded logo and the icons derived from it (D260813b). Created if missing. **The only irreplaceable data.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `CACHE_DIR`                   | `./cache`                                     | WebP derivatives at the root, prepared videos under `CACHE_DIR/video` — two stores, two budgets, two independent LRUs (D6). Regenerable, but not at the same cost: a few seconds of CPU per thumbnail, several minutes per video.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
**All three open with the instance's icon and name.** The icon is
`<img src="${PUBLIC_URL}/api/branding/icon-192.png">` — a PNG, not the SVG the
interface uses, because no mail client renders SVG, and the inbox is the one
place the mark has to survive a hostile renderer. It is also the only remote
resource in a message that is otherwise inline styles and nothing else; a client
that blocks remote images falls back to the `alt` text, which is the instance
name — the very thing the header exists to say. The name is passed in rather than
read from the environment: it is a setting now (D260813c), and `mail.ts` has no
database.
...
host from `PUBLIC_URL` and **never the code** ([D65](./08-decisions/D65-the-code-email-subject-names-the-instance-not-the-code.md)), and it carries
...
HEVC video transcoding (D6), and it's stated outright rather than discovered
...
`plugins/headers.ts` (see [04](./04-security-and-access.md)), so they also apply
...
`/app/config/…` under Docker, `./config/…` in development. `STORAGE_LOCAL_ROOT`
is read the same way, which is why it names `/photos` and not the host directory
mounted there.
...
| `./photos:/photos:ro`     | **Commented out by default.** Photographs already on the machine, for a local-folder storage; uncomment it together with `STORAGE_LOCAL_ROOT=/photos`. The `:ro` is what makes "the application never writes to a storage" a property of the mount rather than of the code. | Not by this application — these are originals it only reads                                      |
| `./Caddyfile:ro`          | Front-end configuration                                                                                                                                                                                                                                                     | No — versioned in the repository                                                                 |
| `lukarn-data`             | `lukarn.db` — **accounts, albums, settings**, index, sessions, encrypted refresh token — and `branding/`, the uploaded logo                                                                                                                                                 | **Yes. It's the only irreplaceable data.**                                                       |
...
writes coming from elsewhere (see [03](./03-data-model.md)).
