#!/usr/bin/env bash
#
# Sauvegarde du volume `nonni-data` et du `.env` qui va avec.
#
...
# NONNI_BACKUP_REMOTE. Aucun secret ne vit ici.

set -euo pipefail

cd "$(dirname "$0")/.."

DESTINATION=${NONNI_BACKUP_DIR:-$PWD/sauvegardes}
# N'importe quel remote rclone convient — S3 et compatibles, Backblaze B2, un
# disque distant en SFTP. `rclone config` le nomme, NONNI_BACKUP_REMOTE le
# désigne ; le dépôt n'en privilégie aucun et n'en connaît aucun secret.
REMOTE=${NONNI_BACKUP_REMOTE:-sauvegardes:nonni}
RETENTION=7
...
if ! docker volume inspect nonni-data >/dev/null 2>&1; then
  echo "Pas de volume nonni-data : instance jamais démarrée, rien à sauvegarder."
  exit 0
fi
...
archive="$DESTINATION/nonni-$horodatage.tar.gz"
secrets="$DESTINATION/nonni-$horodatage.env"
# `.tgz` et non `.tar.gz`, et ce n'est pas une coquetterie : l'élagage distingue
# les archives par motif, et `nonni-*.tar.gz` engloberait celle-ci. La rétention
# tomberait à trois sauvegardes réelles au lieu de sept, sans un message.
configuration="$DESTINATION/nonni-$horodatage.config.tgz"
...
# `-v nonni-data:…` montait alors un volume neuf et vide, et produisait une
...
if ! grep -q 'nonni\.db' <<<"$contenu"; then
  echo "Archive sans nonni.db — le volume monté n'est pas le bon. Rien de sauvegardé." >&2
  rm -f "$archive"
  exit 1
fi
...
elaguer 'nonni-*.tar.gz'
elaguer 'nonni-*.env'
elaguer 'nonni-*.config.tgz'
